...
(function () {
  ...
  function renderOfficialCard(day, stop) {
    ...
    if (!root) return;
    const important = document.getElementById('modalImportantRoot');
    const audit = placeAudit(stop);
    const src = audit && audit.source ? source(audit.source) : null;
    const facts = (audit && audit.facts) || [];

    if (important) {
      important.innerHTML = audit && audit.alert
        ? '<div class="modal-live-alert modal-live-alert-persistent"><b>Important now</b><span>' + escapeHtml(audit.alert) + '</span></div>'
        : '';
    }
    ...
    root.innerHTML =
      '<div class="modal-official-card">' +
        ...
        '<div class="modal-audit-foot">Planner timing stays separate from official operating/access facts. Dynamic conditions must be rechecked before travel.</div>' +
      '</div>';
  }
  ...
  function renderPlacePrereqs(day, stop) {
    ...
    const rows = tasksForStop(stop);
    const done = rows.filter(function (r) { return placeTaskDone(r, stop); }).length;
    const remaining = Math.max(0, rows.length - done);
    const count = document.getElementById('modalPrepareCount');
    if (count) count.textContent = remaining ? String(remaining) : '✓';

    root.innerHTML =
      '<section class="modal-prereq">' +
        '<div class="modal-prereq-head"><div><small>PRE-REQ CHECKLIST • ' + escapeHtml(tripStopCode(day, stop)) + '</small><b>Before this stop</b></div><span>' + done + '/' + rows.length + ' ready</span></div>' +
